...
class ObjectDetectionNode(Node):
    def __init__(self):
        ...
        self.use_adaptive_breakpoint = self.get_parameter(
        ...
        self.frenet_boundary_filter = FrenetBoundaryFilter(
            ...
            right_boundary_inflation=self.get_parameter(
                "right_boundary_inflation"
            ).value,
        ...
        self.object_bboxes_topic = self.get_parameter("object_bboxes_topic").value
            ...
            try:
                ...
                if not self.ego_position_updated:
                    ...
                    self.get_logger().info(
                        f"Ego position initialized: s={s:.2f}, d={d:.2f}, speed={ego_speed:.2f}m/s"
                    )
                    self.ego_position_updated = True
            except Exception as e:
                self.get_logger().error(
                    f"Failed to convert ego position to Frenet: {e}"
                )
        else:
            if not self.ego_position_updated:
                self.get_logger().warn(
                    "Raceline not yet available for ego position conversion",
    ...
    def raceline_callback(self, msg: Path):
        ...
        if self.is_circular_track:
            ...
            self.get_logger().info(
                f"Raceline updated with {len(msg.poses)} points, circular track length: {s:.2f}m"
            )
        else:
            self.get_logger().info(
